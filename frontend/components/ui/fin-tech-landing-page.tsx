...
const softButtonClass =
  "inline-flex items-center justify-center rounded-full px-5 py-2.5 text-sm font-medium shadow-sm transition focus:outline-none focus:ring-2 focus:ring-offset-2 bg-emerald-900 text-white hover:bg-emerald-800 focus:ring-emerald-700";
...
const SoftButton = ({ children, className = "", ...props }: SoftButtonProps) => (
  <button className={`${softButtonClass} ${className}`} {...props}>
    {children}
  </button>
);
...
const SoftLink = ({ href, children, className = "", ...props }: SoftLinkProps) => (
  <Link href={href} className={`${softButtonClass} ${className}`} {...props}>
    {children}
  </Link>
);
...
export default function MrldLandingPage() {
  return (
    <div
      className="min-h-screen w-full bg-[#F3F5F7]"
      style={{
        fontFamily:
          '"Plus Jakarta Sans Variable", "Plus Jakarta Sans", ui-sans-serif, system-ui, -apple-system, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif',
      }}
    >
      {/* Top nav */}
      <nav className="mx-auto flex w-full max-w-[1180px] items-center justify-between px-4 py-6 md:px-0">
        ...
        <div className="hidden items-center gap-8 md:flex">
          {["Çözüm", "Ürün", "Şirket", "İçgörü"].map((item) => (
            <a
              key={item}
              href="#"
        ...
        <div className="hidden gap-2 md:flex">
          <Link
            href="/login"
            className="rounded-full px-4 py-2 text-sm text-slate-700 transition hover:bg-white"
          >
            Giriş
          </Link>
          <SoftLink href="/register">Kayıt Ol</SoftLink>
        </div>
      </nav>
      ...
      <div className="mx-auto grid w-full max-w-[1180px] grid-cols-1 gap-6 px-4 pb-14 md:grid-cols-2 md:px-0">
        ...
        <div className="flex flex-col justify-center space-y-8 pr-2">
          <div>
            <h1 className="text-5xl md:text-6xl font-semibold leading-[1.05] tracking-tight text-slate-900">
              Pazaryerinde
              <br />
              kaybettiğin parayı bul.
            </h1>
            <p className="mt-4 max-w-md text-slate-600">
              <span className="font-medium text-slate-900">MRLD</span>, pazaryeri
              hakedişlerinde komisyon farkı, eksik ödeme ve eksik iadeleri satır
              bazında dakikalar içinde tespit eder.
            </p>
          </div>

          <div className="flex items-center gap-4">
            <SoftLink href="/register">
          ...
          <div className="grid grid-cols-2 gap-8 pt-2 md:max-w-sm">
            <Stat label="Ortalama Kayıp" value="%2 — %7" />
            <Stat label="Tespit Süresi" value="< 5 dk" />
          </div>

          <div className="mt-6 flex items-center gap-8 opacity-70">
            <span className="text-xs text-slate-500">
              DESTEKLENEN PAZARYERLERİ
            </span>
            <div className="flex items-center gap-6 text-slate-400">
              <span className="font-semibold">Trendyol</span>
              <span className="font-semibold">Hepsiburada</span>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          >
            ...
            <div className="relative flex h-full flex-col justify-between">
              ...
              <div className="mt-6 text-xl leading-snug text-emerald-50/95">
                Komisyon, ödeme ve iade kaybını
                <br /> her satırda tespit et
              </div>
              <motion.div
          ...
          >
            ...
            <div className="mt-1 text-xs text-emerald-600 tabular-nums">
              ↑ %2.10 oran · 542 sorun
            </div>
            <MiniBars />
          </motion.div>
      ...
      <footer className="mx-auto w-full max-w-[1180px] px-4 pb-10 text-center text-xs text-slate-400 md:px-0">
        © {new Date().getFullYear()} MRLD — Pazaryeri Gelir Denetim Sistemi.
